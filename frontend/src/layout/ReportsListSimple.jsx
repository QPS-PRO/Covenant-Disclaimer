// frontend/src/layout/ReportsListSimple.jsx
...
const ReportsListSimple = () => {
  ...
  const fetchReports = async () => {
    try {
      ...
      toast.error(t('reportsListSimple.toast.loadFailed'));
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDownloadReport = async (report, format) => {
    ...
    if (
      ...
      if (new Date(transactionFilters.startDate) > new Date(transactionFilters.endDate)) {
        toast.error(t('reportsListSimple.toast.startBeforeEnd'));
        return;
      }
    }
    ...
    try {
      ...
      switch (report.id) {
        ...
        case 'department-summary':
          ...
          break;
        default:
          throw new Error(t('reportsListSimple.toast.unknownType'));
      }

      const filename = reportsUtils.generateFilename(report.id, format);
      reportsUtils.downloadReport(blob, filename);

      toast.success(t('reportsListSimple.toast.downloaded', { name: report.name }));

      // Check if blob is empty or wrong type
      if (blob.size === 0) {
        toast.error(t('reportsListSimple.toast.emptyFile'));
        return;
      }
    } catch (error) {
      console.error('Error downloading report:', error);
      toast.error(t('reportsListSimple.toast.downloadFailed'));
    } finally {
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center min-h-screen">
        ...
        <span className="sr-only">{t('reportsListSimple.loading')}</span>
      </div>
    );
  }
    ...
    <div className="container mx-auto px-4 py-8 max-w-6xl">
      ...
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-gray-900 mb-2">
          {t('reportsListSimple.headerTitle')}
        </h1>
        <p className="text-gray-600">{t('reportsListSimple.headerSubtitle')}</p>
      </div>
            ...
            <thead className="bg-gray-50 border-b border-gray-200">
              <tr>
                <th className="px-6 py-4 text-left text-sm font-semibold text-gray-900">
                  {t('reportsListSimple.table.report')}
                </th>
                <th className="px-6 py-4 text-left text-sm font-semibold text-gray-900">
                  {t('reportsListSimple.table.description')}
                </th>
                <th className="px-6 py-4 text-left text-sm font-semibold text-gray-900">
                  {t('reportsListSimple.table.formats')}
                </th>
                <th className="px-6 py-4 text-right text-sm font-semibold text-gray-900">
                  {t('reportsListSimple.table.actions')}
                </th>
              </tr>
            </thead>
                          ...
                          <div>
                            ...
                            {report.parameters && report.parameters.length > 0 && (
                              <p className="text-xs text-amber-600 flex items-center gap-1 mt-1">
                                ...
                                {t('reportsListSimple.hasOptionalFilters')}
                              </p>
                            )}
                          </div>
                        </div>
                        ...
                        <div className="flex justify-end gap-2">
                          {report.formats.includes('pdf') && (
                            <button
                              ...
                              title={t('reportsListSimple.downloadPdfTitle')}
                            >
                          ...
                          {report.formats.includes('excel') && (
                            <button
                              ...
                              title={t('reportsListSimple.downloadExcelTitle')}
                            >
                        ...
                        <td colSpan="4" className="px-6 py-4">
                          <div className="flex items-center gap-4">
                            ...
                            <span className="text-sm font-medium text-gray-700">
                              {t('reportsListSimple.dateRangeOptional')}
                            </span>
                            <input
                              type="date"
                              ...
                              placeholder={t('reportsListSimple.startDate')}
                              className="px-3 py-2 border border-gray-300 rounded-md text-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                            />
                            <span className="text-gray-400">{t('reportsListSimple.to')}</span>
                            <input
                              type="date"
                              ...
                              placeholder={t('reportsListSimple.endDate')}
                              className="px-3 py-2 border border-gray-300 rounded-md text-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                            />
      ...
      {reports.length === 0 && (
        <div className="text-center py-12 bg-white rounded-lg shadow-sm border border-gray-200">
          ...
          <h3 className="text-lg font-medium text-gray-900 mb-2">
            {t('reportsListSimple.emptyTitle')}
          </h3>
          <p className="text-gray-600">{t('reportsListSimple.emptyBody')}</p>
        </div>
      )}
    </div>
  );
