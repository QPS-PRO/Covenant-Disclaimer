...
class EmployeeSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        extra_kwargs = {"face_recognition_data": {"write_only": True}}
...
class AssetSerializer(serializers.ModelSerializer):
    ...
    current_holder_name = serializers.CharField(
        source="current_holder.name", read_only=True
    )
    current_holder_employee_id = serializers.CharField(
        source="current_holder.employee_id", read_only=True
    )
    ...
    def validate(self, data):
        ...
        if status_effective == "assigned" and not holder_effective:
            raise serializers.ValidationError("Asset marked as assigned must have a current holder.")
        elif status_effective != "assigned" and holder_effective:
            # clear holder if not assigned
...
class AssetTransactionCreateSerializer(serializers.ModelSerializer):
    ...
    def validate(self, attrs):
        ...
        print(
            f"Validating transaction - Employee: {employee.name if employee else None}, Asset: {asset.name if asset else None}"
        )
        ...
        if transaction_type == "issue" and asset.status != "available":
            raise serializers.ValidationError(
                f"Asset '{asset.name}' is not available for issue (current status: {asset.status})"
            )

        if transaction_type == "return":
            if asset.status != "assigned":
                raise serializers.ValidationError(
                    f"Asset '{asset.name}' is not currently assigned (current status: {asset.status})"
                )
            if asset.current_holder != employee:
                ...
                raise serializers.ValidationError(
                    f"Asset '{asset.name}' is not assigned to {employee.name} (currently assigned to: {current_holder_name})"
                )
        ...
        if employee and employee.face_recognition_data:
            ...
            try:
                verification_result = verify_employee_face(
                    employee, face_verification_data
                )
                ...
                attrs["face_verification_confidence"] = verification_result.get(
                    "confidence", 0.0
                )
                print(
                    f"Face verification successful with confidence: {verification_result.get('confidence', 0.0):.1%}"
                )

            except Exception as e:
                print(f"Face verification error: {str(e)}")
                raise serializers.ValidationError(
                    f"Face verification failed due to technical error: {str(e)}"
                )
            ...
            print(
                f"Employee {employee.name if employee else 'Unknown'} has no face data, marking as not verified"
            )
    ...
    def validate(self, data):
        try:
            ...
            if not employee.face_recognition_data:
                raise serializers.ValidationError(
                    "Employee has no registered face data"
                )
...
class AssetReturnSerializer(serializers.Serializer):
    ...
    def validate(self, data):
        ...
        try:
            ...
            if asset.status != "assigned":
                raise serializers.ValidationError(
                    f"Asset is not currently assigned (status: {asset.status})"
                )
            data["asset"] = asset
        ...
        if data["return_condition"] in ["Damaged", "Poor"] and not data.get(
            "damage_notes"
        ):
            raise serializers.ValidationError(
